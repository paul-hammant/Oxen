...
pub fn new_for_url_with_bearer_token<U: IntoUrl>(
    url: U,
    bearer_token: &str,
) -> Result<Client, OxenError> {
    let (_scheme, host) = get_scheme_and_host_from_url(url)?;
...
fn new_for_host_with_bearer_token<S: AsRef<str>>(
    host: S,
    bearer_token: &str,
    ...
    fn test_get_scheme_and_host_from_url() {
        let test_cases = vec![
            ...
            (
                ...
                ("https".to_string(), "test.example.com:8443".to_string()),
            ),
        ];
