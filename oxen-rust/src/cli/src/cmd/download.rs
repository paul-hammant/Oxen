...
impl RunCmd for DownloadCmd {
    ...
    async fn run(&self, args: &clap::ArgMatches) -> Result<(), OxenError> {
            ...
            api::client::repositories::get_by_name_host_and_scheme(id, &host, &scheme).await?
        {
            ...
            for path in paths {
                repositories::download_with_bearer_token(
                    &remote_repo,
                    &path,
                    &dst,
                    &revision,
                    bearer_token.as_ref().map(|s| s.as_str()),
                )
                .await?;
            }
        } else {
...
mod tests {
    ...
    fn create_download_args_with_bearer_token(
        ...
        let mut args = vec![OsString::from("download"), OsString::from(id)];
    ...
    fn test_download_cmd_args_with_bearer_token() {
        ...
        let bearer_token_arg = command
            .get_arguments()
            .find(|arg| arg.get_id() == "bearer_token");
    ...
    fn test_download_cmd_parse_args_with_bearer_token() {
        ...
        assert_eq!(
            args.get_one::<String>("bearer_token").unwrap(),
    ...
    fn test_download_cmd_bearer_token_is_optional() {
        ...
        let args =
            create_download_args_with_bearer_token("ox/test", vec!["file.txt"], None, None, None);
        assert!(args.get_one::<String>("bearer_token").is_none());
